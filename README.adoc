...
If you want to try the kafka-exporter, copy the .env-template to .env and customize it.
Then enable the docker compose profile by setting:

```console
export COMPOSE_PROFILES=kafka-exporter
````
